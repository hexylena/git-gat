...
# CMD
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/customization/0005" | cut -c1-40)
## Run command
if [[ -z ${GALAXY_VERSION} ]]; then
ansible-playbook galaxy.yml -i ~/.hosts --vault-password-file ~/.vault-password.txt
else
...
# CMD
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/customization/0009" | cut -c1-40)
## Run command
if [[ -z ${GALAXY_VERSION} ]]; then
ansible-playbook galaxy.yml -i ~/.hosts --vault-password-file ~/.vault-password.txt
else
