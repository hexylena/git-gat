...
# CMD
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/pulsar/0006" | cut -c1-40)
## Run command
if [[ -z ${GALAXY_VERSION} ]]; then
ansible-playbook galaxy.yml -i ~/.hosts --vault-password-file ~/.vault-password.txt
else
...
# CMD
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/pulsar/0010" | cut -c1-40)
## Run command
if [[ -z ${GALAXY_VERSION} ]]; then
...
# CMD
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/pulsar/0014" | cut -c1-40)
## Run command
if [[ -z ${GALAXY_VERSION} ]]; then
ansible-playbook galaxy.yml -i ~/.hosts --vault-password-file ~/.vault-password.txt
else
...
git checkout $(git log main --pretty=oneline | grep "admin/pulsar/0014" | cut -c1-40)
## Run test case
./.scripts/20-pulsar-test/1-pulsar.sh

# TEST
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/pulsar/0014" | cut -c1-40)
## Run test case
./.scripts/20-pulsar-test/2-run-job.sh
# Done!
git checkout main
