...
# CMD
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/backup-cleanup/0000" | cut -c1-40)
## Run command
ansible-galaxy install -p roles -r requirements.yml

# CMD
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/backup-cleanup/0003" | cut -c1-40)
## Run command
...
# CMD
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/backup-cleanup/0006" | cut -c1-40)
## Run command
if [[ -z ${GALAXY_VERSION} ]]; then
ansible-playbook galaxy.yml -i ~/.hosts --vault-password-file ~/.vault-password.txt
else
