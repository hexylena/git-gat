...
# CMD
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/beacon/0006" | cut -c1-40)
## Run command
...
# CMD
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/beacon/0008" | cut -c1-40)
## Run command
if [[ -z ${GALAXY_VERSION} ]]; then
ansible-playbook beacon-import.yml -i ~/.hosts --vault-password-file ~/.vault-password.txt
else
ansible-playbook beacon-import.yml -i ~/.hosts --vault-password-file ~/.vault-password.txt -e galaxy_commit_id=${GALAXY_VERSION}
fi

# CMD
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/beacon/0010" | cut -c1-40)
## Run command
if [[ -z ${GALAXY_VERSION} ]]; then
