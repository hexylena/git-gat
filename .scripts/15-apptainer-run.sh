...
# CMD
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/apptainer/0000" | cut -c1-40)
## Run command
ansible-galaxy install -p roles -r requirements.yml

# CMD
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/apptainer/0002" | cut -c1-40)
## Run command
...
# CMD
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/apptainer/0004" | cut -c1-40)
## Run command
mkdir -p templates/galaxy/config

# CMD
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/apptainer/0008" | cut -c1-40)
## Run command
if [[ -z ${GALAXY_VERSION} ]]; then
ansible-playbook galaxy.yml -i ~/.hosts --vault-password-file ~/.vault-password.txt
else
...
git checkout $(git log main --pretty=oneline | grep "admin/apptainer/0008" | cut -c1-40)
## Run test case
./.scripts/15-apptainer-test/1-run-minimap2.sh
