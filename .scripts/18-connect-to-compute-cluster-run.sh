...
# CMD
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/connect-to-compute-cluster/0000" | cut -c1-40)
## Run command
ansible-galaxy install -p roles -r requirements.yml

# CMD
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/connect-to-compute-cluster/0003" | cut -c1-40)
## Run command
...
# CMD
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/connect-to-compute-cluster/0005" | cut -c1-40)
## Run command
if [[ -z ${GALAXY_VERSION} ]]; then
ansible-playbook galaxy.yml -i ~/.hosts --vault-password-file ~/.vault-password.txt
else
...
# CMD
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/connect-to-compute-cluster/0007" | cut -c1-40)
## Run command
if [[ -z ${GALAXY_VERSION} ]]; then
ansible-playbook galaxy.yml -i ~/.hosts --vault-password-file ~/.vault-password.txt
else
...
git checkout $(git log main --pretty=oneline | grep "admin/connect-to-compute-cluster/0007" | cut -c1-40)
## Run test case
./.scripts/18-connect-to-compute-cluster-test/1-test-cat1.sh

# CMD
## Checkout
git checkout $(git log main --pretty=oneline | grep "admin/connect-to-compute-cluster/0010" | cut -c1-40)
## Run command
if [[ -z ${GALAXY_VERSION} ]]; then
ansible-playbook galaxy.yml -i ~/.hosts --vault-password-file ~/.vault-password.txt
else
