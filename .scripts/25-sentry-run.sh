...
ansible-playbook sentry.yml galaxy.yml -i ~/.hosts --vault-password-file ~/.vault-password.txt
else
ansible-playbook sentry.yml galaxy.yml -i ~/.hosts --vault-password-file ~/.vault-password.txt -e galaxy_commit_id=${GALAXY_VERSION}
fi
