...
## Run command
if [[ -z ${GALAXY_VERSION} ]]; then
ansible-playbook pulsar.yml -i ~/.hosts --vault-password-file ~/.vault-password.txt -e "nginx_ssl_role=${SSL_ROLE} openssl_domains={{ certbot_domains }}" 
else
ansible-playbook pulsar.yml -i ~/.hosts --vault-password-file ~/.vault-password.txt -e "nginx_ssl_role=${SSL_ROLE} openssl_domains={{ certbot_domains }} galaxy_commit_id=${GALAXY_VERSION}"
fi
